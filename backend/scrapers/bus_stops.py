...
routes_payload = {
    "key":api_key,
    "stop_id":"" #change stop_id to id of stop
}
...
def get_full_stop_info(stop_data, payload, url):
    ...
    for stop_id in stop_data.keys():
        ...
        for stop_route in single_stop_routes_raw["routes"]:
            route_name = stop_route["route_short_name"] + " " + stop_route["route_long_name"]
        ...
        print(stop_id, "finished.", stop_counter, "stops processed.", str(perc_complete)[:4] + "% complete.") #debugging
    return stop_data
